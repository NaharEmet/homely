...
| U2 | ui-plan-enhance | U1 | homely/src/plan/renderer.ts + homely/src/style.css | clone-dev | clone-dev | review | Plan view polish: grid background (major/minor lines), wall thickness visible as filled shapes not stroked centerlines, room floor fill + area label centered, selection handles (blue outlines + corner dots), zoom-to-fit with scroll wheel, pan with middle-click/Space+drag. |
...
| U4 | ui-3d-polish | U1 | homely/src/view3d/scene.ts + homely/src/view3d/view.ts | clone-dev | clone-dev | in_progress | 3D view polish: wall side textures (leftSideColor/rightSideColor), floor+ceiling for rooms, shadow mapping, better ground plane with grid lines, orbit controls (orbit/pan/zoom), ambient occlusion hint, fog for distance. |
