...
before Track M existed) is already working overlapping ground via its own
git worktrees: `wt-renderer` (`fix/renderer-yaw`), `wt-furniture`
(`fix/furniture-render`, already 2 commits in), `wt-doors`
(`fix/doors-windows`), `wt-lint` (`fix/lint-config`). Per user decision:
**do not duplicate that work.** M2, M5, and the lint-config half of M10 are
reassigned from "dispatch to opencode" to "verify + merge" — the manager
independently re-runs each worktree branch's tests/e2e/lint before merging
it into `main`, same gatekeeper bar as everything else on this board, but
does not spin up a competing opencode worker for that territory. M10 is
narrowed to CI-pipeline scaffolding only (workflow/script wiring), not
touching lint config itself.
...
| M5 | doors-and-windows | M1,M3 | homely/src/plan + homely/src/core + homely/src/view3d | verify:wt-doors | | todo | P1. **Reassigned to verify+merge** (see coordination note above) — `wt-doors`/`fix/doors-windows` already covers this. Original spec kept below for the manager's verification checklist. Dropping a door/window catalog piece (`eTeks#door`, `eTeks#window85x123`, etc., already in `catalog.json`) onto a wall must cut/reserve an opening in that wall (SH3D "piece on wall" semantics) in both plan and 3D, and the piece must move/resize with the wall. Add automation commands so `qa-loop/scenarios/doors_windows.yaml` passes (currently `UNKNOWN_COMMAND` for `add_door`/`add_window` per `qa-loop/MISSING_FEATURES.md`). DoD: `cd qa-loop && python3 run.py doors_windows` (or equivalent) reports 0 errors; e2e places a door on a wall and the wall visibly shows a gap under it. |
