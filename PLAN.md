...
| B8 | export-parity-defaults | B5 | homely/src | clone-dev | clone-dev | review | All 6 gaps fixed vs driver golden create_room.expected-state.json: thickness 7; addWallChain stamps patternId 'hatchUp'; wallsAlpha default 0 (+ view3d now treats it as TRANSPARENCY per Wall3D.java:1522, was inverted); compass lat/lon from OS TZ via full 603-entry Compass.java port (homely/src/core/compass-timezones.ts, float32-exact pipeline, zone injectable for tests) exported round3; camera ids 'camera-top-1'/'camera-observer-1' + observer fixedSize:false in export; normalizeAngle wraps [-180,180) so yaw π exports -180.0, fovDeg plain round3. DEVIATION: kickoff/contract §3 said UTC/Etc/GMT→(0,0), but Compass.java:673 maps them to Greenwich Observatory (51.466667,0)deg → latRad≈0.898; implemented the faithful table, flagged for integrator doc fix. Verified: vitest 109/109 (99+10 new incl golden-parity replay, Thimphu 0.48/1.564 + Kolkata values, angle wrap, thickness/pattern/wallsAlpha/camera ids), tsc clean, eslint clean, vite build ok. |
