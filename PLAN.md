...
| B6 | plan-parity-autoroom | B5 | homely/src/plan | clone-dev | | todo | W3 integration finding: SH3D 7.5 validateDrawnWalls NEVER creates rooms on wall-loop close (PlanController.java:10912 only posts+selects walls; rooms come only from the room tool, createRoom callers :14139/:14155). Homely engine doubleClick->addWallChain(withRoom=closesCycle) auto-creates one -> state diff vs driver. Fix: wall tool must not create rooms; update B3 tests/equivalence script accordingly. Driver-side conformance fixes already landed by integrator (click dbl:true honored; protocol tool names wall/room/dimensionLine accepted). |
