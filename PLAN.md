...
| B7 | top-camera-follower | B5 | homely/src/core | clone-dev | clone-dev | review | Port SH3D HomeController3D$TopCameraState (HomeController3D.java:823-1330): top camera follows whole-home bounds on item add/delete + property changes. Full algorithm + numeric verification in docs/behaviours/sh3d-camera-and-export.md. Must reproduce driver-observed positions for the 4-wall script exactly ((50,1050,1010,-180) fresh → (350,1250,1125) final). Done: follower in core/top-camera-follower.ts wired into HomeStore apply/undo/redo; mitered thick-polygon wall corners (geometric join detection) reproduce contract table incl. row-3 (351.75,248.25) at t=7; camera-only mutations skipped; reset=new default camera; 99/99 vitest (84+15 new), tsc+eslint clean, vite build ok. Deviations: NEW_WALL_THICKNESS_CM still 7.5 (B8); arc walls bounds as straight; selection path skipped per contract. |
| B8 | export-parity-defaults | B5 | homely/src | clone-dev | | todo | State-export + defaults parity vs real driver golden: new-wall thickness 7cm (not 7.5); default wall patternId 'hatchUp'; HomeEnvironment wallsAlpha default 0; compass latRad/lonRad from OS timezone via SH3D Compass.java TZ→coords table (Intl.DateTimeFormat().resolvedOptions().timeZone); camera ids 'camera-top-1'/'camera-observer-1' in export; export rounding = round3 HALF_EVEN for x/y/z/fovDeg, angles wrapped to (-180,180] then round3 (yaw π → -180.0). Details in docs/behaviours/sh3d-camera-and-export.md. |
...
| C8 | comparator-null-normalize | C3 | equivalence/eq/comparators | harness-dev | harness-dev | review | Deep-diff now treats absent key == null value (both directions, deep_diff + compare_states); real null/value diffs still fail. Replay of live run 20260826-105922: 21 false positives gone; 18 genuine B7/B8 parity gaps remain (thickness 7.0vs7.5, patternId, compass, top camera pos). 98/98 pytest (6 new), ruff clean. Goldens untouched. |
| D2 | golden-and-slice | C7,C8,B7,B8 | equivalence/scenarios/slice | grace | | todo | vertical slice YAMLs under equivalence/scenarios/slice/; golden .sh3d via driver + committed expected-state JSONs (DONE: create_room.yaml + goldens committed from real driver :9450); live E2E run 1 found parity gaps -> triaged into C8/B7/B8 (see docs/behaviours/sh3d-camera-and-export.md). DoD: ./test-equivalence slice/create_room end-to-end both apps PASS (0 state diffs, assertions green) |
